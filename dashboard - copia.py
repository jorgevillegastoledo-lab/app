...
def dashboard():
    ...
    # Obtener préstamos activos (con cuotas por pagar)
    prestamos_activos = df_prestamos[df_prestamos["cuotas_pagadas"] < df_prestamos["cuotas_totales"]].copy()

    # Filtrar préstamos con pagos registrados en el mes actual o anteriores
    ids_prestamos_validos = df_pagos_prestamo[df_pagos_prestamo["mes_contable"] <= mes_actual]["prestamo_id"].unique()
    prestamos_activos = prestamos_activos[prestamos_activos["id"].isin(ids_prestamos_validos)]
    
    # Filtrar préstamos con pagos registrados en el mes actual o anterior
    #ids_prestamos_validos = df_pagos_prestamo[df_pagos_prestamo["mes_contable"] <= mes_actual]["prestamo_id"].unique()
    #prestamos_activos = prestamos_activos[prestamos_activos["id"].isin(ids_prestamos_validos)]
    ...
    # Filtrar préstamos que no han terminado de pagarse
    prestamos_activos = df_prestamos[df_prestamos["cuotas_pagadas"] < df_prestamos["cuotas_totales"]].copy()

    # ✅ NUEVO: filtrar según mes de inicio del préstamo
    mes_inicio = list(meses_dict.keys())[list(meses_dict.values()).index(mes_nombre)]
    #prestamos_activos = prestamos_activos[prestamos_activos["mes_pago"] <= mes_inicio]
